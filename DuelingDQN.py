...
class DuelingDQN:
    ...
    def __init__(self):
        ...
        down3 = Flatten()(down2)
        v_dense = Dense(256, activation='relu')(down3)
        v_out = Dense(1)(v_dense)
        adv_dense = Dense(256, activation='relu')(down3)
        adv_out = Dense(GameConstants.ACTION_SPACE_SIZE)(adv_dense)
        norm_adv = Lambda(lambda a: a[:, :] - K.mean(a[:, :], keepdims=True),
                                  output_shape=(GameConstants.ACTION_SPACE_SIZE,))(adv_out)
        X = Add()([v_out, norm_adv])
    ...
    def train_double_dqn(self, minibatch):
        ...
        loss = [mse(target_q[i], main_output[i]) for i in range(len(main_output))]
        error = [loss[i].numpy() for i in range(len(loss))]
